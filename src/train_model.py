"""
AChE Inhibitor Prediction for Alzheimer's Disease
==================================================
Pipeline: ChEMBL data curation -> Morgan fingerprints -> feature selection
-> benchmarking of 4 ML classifiers (KNN, SVM, Random Forest, XGBoost).

Authors: Dora Diaz, Marwan El Saabi, Juan Jimenez, Nerea Portabales
"""
...
# 1. LOAD DATA
# ----------------------------------------------------------------------
# Raw data is not included in this repository (see data/README.md for
# instructions on how to regenerate it from ChEMBL).
FILE_NAME = 'data/raw/datos_alzheimer.csv'
print(f"\nLoading file: {FILE_NAME}")

try:
    # ChEMBL exports commonly use ';' as separator
    ...
    # ------------------------------------------------------------------
    # 3. COMPOUND CLASSIFICATION AND BALANCING
    # ------------------------------------------------------------------
    def classify_compound(value):
        if value <= 1000:
            ...
            return 0  # Inactive
        return -1  # Ambiguous ("grey zone"), removed
    ...
    # ------------------------------------------------------------------
    # 4. FEATURIZATION (MORGAN FINGERPRINTS)
    # ------------------------------------------------------------------
    ...
    for name, clf_model in classifiers.items():
        pipeline = Pipeline([
            ...
            ('feature_selector', SelectKBest(score_func=f_classif, k=300)),  # k tuned via sensitivity analysis
    ...
    print(f"\nERROR: The file '{FILE_NAME}' was not found. See data/README.md to regenerate it from ChEMBL.")
